...
infile <- "/Users/ying/Work/BCCRC/Projects/colon/data/mutations/tp53_mutant.txt"
tp53_data <- read.table(infile, header=FALSE, sep="\t")
tp53_samples <- unique(tp53_data[,1])
tp53_samples <- tp53_samples[tp53_samples %in% sample_list]
tp53_normals <- tp53_samples + 1
...
tp53_index <- which(sample_list %in% c(tp53_samples, tp53_normals))
nonTP53_index <- which(!sample_list %in% c(tp53_samples, tp53_normals))
sample_list_tp53 <- sample_list[c(tp53_index, nonTP53_index)]

png(paste("../../results/", threshold, "/chimExp_bar_LINE_tp53.png", sep=""), width=16, height=8, units="in", res=300)
...
LINE_bySample_tp53 <- LINE_bySample[c(tp53_index, nonTP53_index)]

barplot(matrix(LINE_bySample_tp53, 2, length(sample_list)/2), beside=TRUE, col=colorset, xlab="Samples", ylab="Number of Chimeras")
axis(1, tick=FALSE, labels=FALSE)
text(x=sort(c(seq(1,187,3), seq(2,188,3)))+0.3, y=par("usr")[3]-0.2, srt=90, adj = 0.8, labels = sample_list_tp53, cex=0.5, xpd = TRUE)
...
png(paste("../../results/", threshold, "/chimExp_bar_SINE_tp53.png", sep=""), width=16, height=8, units="in", res=300)
par(mgp=c(3,1,0), mar=c(5,5,4,2)+0.1) # mgp sets the distance of axis labels to the axis; mar sets the size of figure margin
colorset <- c("darkblue", "orange")
SINE_bySample_tp53 <- SINE_bySample[c(tp53_index, nonTP53_index)]

barplot(matrix(SINE_bySample_tp53, 2, length(sample_list)/2), beside=TRUE, col=colorset, xlab="Samples", ylab="Number of Chimeras")
axis(1, tick=FALSE, labels=FALSE)
text(x=sort(c(seq(1,187,3), seq(2,188,3)))+0.3, y=par("usr")[3]-0.2, srt=90, adj = 0.8, labels = sample_list_tp53, cex=0.5, xpd = TRUE)
...
png(paste("../../results/", threshold, "/chimExp_bar_LTR_tp53.png", sep=""), width=16, height=8, units="in", res=300)
par(mgp=c(3,1,0), mar=c(5,5,4,2)+0.1) # mgp sets the distance of axis labels to the axis; mar sets the size of figure margin
colorset <- c("darkblue", "orange")
LTR_bySample_tp53 <- LTR_bySample[c(tp53_index, nonTP53_index)]

barplot(matrix(LTR_bySample_tp53, 2, length(sample_list)/2), beside=TRUE, col=colorset, xlab="Samples", ylab="Number of Chimeras")
axis(1, tick=FALSE, labels=FALSE)
text(x=sort(c(seq(1,187,3), seq(2,188,3)))+0.3, y=par("usr")[3]-0.2, srt=90, adj = 0.8, labels = sample_list_tp53, cex=0.5, xpd = TRUE)
...
#================================================================================
# Barplot of chimeria fold change

LINE_tp53_fc <- matrix(LINE_bySample_tp53, 2, length(sample_list)/2)
LINE_tp53_fc <- LINE_tp53_fc[1,]/LINE_tp53_fc[2,]

png(paste("../../results/", threshold, "/chimExpFC_LINE_tp53.png", sep=""), width=16, height=8, units="in", res=300)
par(mgp=c(3,1,0), mar=c(5,5,4,2)+0.1) # mgp sets the distance of axis labels to the axis; mar sets the size of figure margin
can_samples <- sample_list_tp53[c(TRUE, FALSE)]
colorset <- c(rep("red", length(tp53_samples)), rep("blue", length(sample_list)/2 - length(tp53_samples)))
barplot(LINE_tp53_fc, las=2, names.arg=can_samples, col=colorset, cex.names=0.7, xlab="Samples", ylab="Fold-Change of Chimera Expression w.r.t. Normal Samples")
#axis(1, tick=FALSE, labels=FALSE)
#text(x=1:length(sample_list)/2, y=par("usr")[3], srt=90, adj = 0.8, labels = can_samples, cex=0.5, xpd = TRUE)
y_lim <- max(c(LINE_tp53_fc))
legend(1, y_lim, c("TP53_mutant", "TP53_normal"), c("red", "blue"))
dev.off()

SINE_tp53_fc <- matrix(SINE_bySample_tp53, 2, length(sample_list)/2)
SINE_tp53_fc <- SINE_tp53_fc[1,]/SINE_tp53_fc[2,]

png(paste("../../results/", threshold, "/chimExpFC_SINE_tp53.png", sep=""), width=16, height=8, units="in", res=300)
par(mgp=c(3,1,0), mar=c(5,5,4,2)+0.1) # mgp sets the distance of axis labels to the axis; mar sets the size of figure margin
can_samples <- sample_list_tp53[c(TRUE, FALSE)]
colorset <- c(rep("red", length(tp53_samples)), rep("blue", length(sample_list)/2 - length(tp53_samples)))
barplot(SINE_tp53_fc, las=2, names.arg=can_samples, col=colorset, cex.names=0.7, xlab="Samples", ylab="Fold-Change of Chimera Expression w.r.t. Normal Samples")
#axis(1, tick=FALSE, labels=FALSE)
#text(x=1:length(sample_list)/2, y=par("usr")[3], srt=90, adj = 0.8, labels = can_samples, cex=0.5, xpd = TRUE)
y_lim <- max(c(SINE_tp53_fc))
legend(1, y_lim, c("TP53_mutant", "TP53_normal"), c("red", "blue"))
dev.off()

LTR_tp53_fc <- matrix(LTR_bySample_tp53, 2, length(sample_list)/2)
LTR_tp53_fc <- LTR_tp53_fc[1,]/LTR_tp53_fc[2,]

png(paste("../../results/", threshold, "/chimExpFC_LTR_tp53.png", sep=""), width=16, height=8, units="in", res=300)
par(mgp=c(3,1,0), mar=c(5,5,4,2)+0.1) # mgp sets the distance of axis labels to the axis; mar sets the size of figure margin
can_samples <- sample_list_tp53[c(TRUE, FALSE)]
colorset <- c(rep("red", length(tp53_samples)), rep("blue", length(sample_list)/2 - length(tp53_samples)))
barplot(LTR_tp53_fc, las=2, names.arg=can_samples, col=colorset, cex.names=0.7, xlab="Samples", ylab="Fold-Change of Chimera Expression w.r.t. Normal Samples")
#axis(1, tick=FALSE, labels=FALSE)
#text(x=1:length(sample_list)/2, y=par("usr")[3], srt=90, adj = 0.8, labels = can_samples, cex=0.5, xpd = TRUE)
y_lim <- max(c(LTR_tp53_fc))
legend(1, y_lim, c("TP53_mutant", "TP53_normal"), c("red", "blue"))
dev.off()
